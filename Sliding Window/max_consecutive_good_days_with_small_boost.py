...
def max_consecutive_good_days_with_small_boost(sales, k):
    # create left pointer
    left = 0 
    # create number of boostable days as boostable
    boostable = 0 
    # create best window as max days 
    max_days = 0
    for right in range(len(sales)):
        ...
        if curr_window > max_days:
            max_days = curr_window 
